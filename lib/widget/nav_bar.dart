...
/// to also has a [SupportNavigationBar] or a [SupportSliverNavigationBar]
...
///
/// By default, only one [SupportNavigationBar] or [SupportSliverNavigationBar]
...
/// This example shows a [SupportNavigationBar] placed in a [CupertinoPageScaffold].
...
///  * [SupportSliverNavigationBar] for a navigation bar to be placed in a
///    scrolling list and that supports iOS-11-style large titles.
class SupportNavigationBar extends StatefulWidget implements ObstructingPreferredSizeWidget {
  /// Creates a navigation bar in the iOS style.
  const SupportNavigationBar({
    Key key,
    this.leading,
  ...
  ///
  /// Defaults to a common tag between all [SupportNavigationBar] and
  /// [SupportSliverNavigationBar] instances of the same [Navigator]. With the
  ...
  @override
  _SupportNavigationBarState createState() {
    return _SupportNavigationBarState();
  }
}
...
class _SupportNavigationBarState extends State<SupportNavigationBar> {
  ...
  @override
  Widget build(BuildContext context) {
    final backgroundColor = CupertinoDynamicColor.maybeResolve(widget.backgroundColor, context) ?? CupertinoTheme.of(context).barBackgroundColor;
    ...
    final navBar = _wrapWithBackground(
...
///
/// The [SupportSliverNavigationBar] must be placed in a sliver group such
...
///
/// Like [SupportNavigationBar], it also supports a [leading] and [trailing]
...
/// See also:
///
///  * [SupportNavigationBar], an iOS navigation bar for use on non-scrolling
///    pages.
class SupportSliverNavigationBar extends StatefulWidget {
  /// Creates a navigation bar for scrolling lists.
  ///
  /// The [largeTitle] argument is @required and must not be null.
  const SupportSliverNavigationBar({
    Key key,
    this.largeTitle,
    this.leading,
    this.automaticallyImplyLeading = true,
  ...
  @override
  _SupportSliverNavigationBarState createState() => _SupportSliverNavigationBarState();
}

// A state class exists for the nav bar so that the keys of its sub-components
// don't change when rebuilding the nav bar, causing the sub-components to
// lose their own states.
class _SupportSliverNavigationBarState extends State<SupportSliverNavigationBar> {
  _NavigationBarStaticComponentsKeys keys;
...
class _LargeTitleNavigationBarSliverDelegate extends SliverPersistentHeaderDelegate with DiagnosticableTreeMixin {
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    final showLargeTitle = shrinkOffset < maxExtent - minExtent - _kNavBarShowLargeTitleThreshold;

    final persistentNavigationBar = _PersistentNavigationBar(
      components: components,
      padding: padding,
...
/// A nav bar back button typically used in [SupportNavigationBar].
///
/// This is automatically inserted into [SupportNavigationBar] and
/// [SupportSliverNavigationBar]'s `leading` slot when
/// `automaticallyImplyLeading` is true.
///
...
class _NavigationBarComponentsTransition {
  ...
  RelativeRectTween slideFromLeadingEdge({
    ...
    final fromRect = positionInTransitionBox(fromKey, from: fromNavBarBox);
  ...
  Widget get bottomBackLabel {
    ...
    final from = positionInTransitionBox(bottomComponents.backLabelKey, from: bottomNavBarBox);
  ...
  Widget get bottomLargeTitle {
    ...
    if (bottomLargeTitle != null && topLeading != null) {
      ...
      final from = positionInTransitionBox(bottomComponents.largeTitleKey, from: bottomNavBarBox);
  ...
  Widget get topBackChevron {
    ...
    final to = positionInTransitionBox(topComponents.backChevronKey, from: topNavBarBox);
    var from = to;
  ...
  Widget get topBackLabel {
    ...
    final topBackLabelOpacity = topComponents.backLabelKey.currentContext?.findAncestorRenderObjectOfType<RenderAnimatedOpacity>();
  ...
  Widget get topLargeTitle {
    ...
    final to = positionInTransitionBox(topComponents.largeTitleKey, from: topNavBarBox);
